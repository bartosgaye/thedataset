...
function [A,Y]=augment(X,Y)
    A=zeros(28,28,1,size(X,4));
    for i=1:size(X,4)
       I=X(:,:,:,i);
       c=randi([1 3]);
       if c==1
...
function D=pinch(I)
    amt = -(randi([1 9])/10);
    D = imbinarize(lensdistort(I,amt,'padmethod','replicate'));
end
function D=barrel(I)
    amt=randi([1 9])/10;
    D=imbinarize(lensdistort(I,amt,'padmethod','replicate'));
...
function D=tilt_right(I)
    ...
    fixedPoints  = [first second; 0 nrows; ncols 0; ncols nrows]; 
    t_piecewise_linear = fitgeotrans(movingPoints,fixedPoints,'pwl'); 
    D = imbinarize(imwarp(I,t_piecewise_linear,'FillValues',fill));
end
function D=tilt_left(I)
    nrows=28;
    ncols=28;
    fill=0;
    first=randi([1 5])/10+1;
    second=randi([1 5])/10+1;
    movingPoints = [0 0; 0 nrows; ncols 0; ncols nrows;]; 
    fixedPoints  = [0 0; 0 nrows; ncols 0; ncols*first nrows*second]; 
    t_piecewise_linear = fitgeotrans(movingPoints,fixedPoints,'pwl'); 
    D = imbinarize(imwarp(I,t_piecewise_linear,'FillValues',fill));
end
function D=rot_left(I)
    ang=randi([1 5])*5;
    tmp=imrotate(I,ang,'nearest','loose');
    D=imresize(tmp,[28 28]);
end
function D=rot_right(I)
    ang=randi([1 5])*5;
    tmp=imrotate(I,-ang,'nearest','loose');
    D=imresize(tmp,[28 28]);
end